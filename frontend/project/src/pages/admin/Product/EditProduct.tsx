...
export interface FormValuesUpdateProduct {
  productId: string;
  productName: string;
  price: number;
  salePrice: number;
...
const EditProduct: React.FC = () => {
  ...
  const { data: product, isPending: isPendingProduct, isFetching } = useProductByID();

  const { register, handleSubmit, formState: { errors }, setValue, getValues, reset } = useForm<FormValuesUpdateProduct>({
    defaultValues: {
      ...
      brand: product?.data.productInfo.brand,
      cpu: product?.data.productInfo.cpu,
      screenSize: product?.data.productInfo.screenSize,
      ram: product?.data.productInfo.ram,
      rom: product?.data.productInfo.rom,
      batteryLife: product?.data.productInfo.batteryLife,
    }
  });
  ...
  const handleAddProduct: SubmitHandler<FormValuesUpdateProduct> = (data) => {
    // console.log(data);
    updateProduct({ ...data, productId: product?.data.productId });
  }
  ...
  return (
    <div className='py-4'>
      ...
      <h1 className='text-4xl font-semibold text-center text-blue-500 mb-4'>Edit product</h1>
            ...
            <div className='flex flex-col gap-1'>
              ...
              <div className='flex flex-col justify-between lg:flex-row gap-2'>
                <div className='flex flex-col mb-3 space-y-2 w-[100%]'>
                  ...
                  <input {...register('quantity')} type='text' placeholder='Quantity' className='focus:outline-none rounded-xl px-3 py-2 border w-full text-base focus:ring-2' />
                  {/* {errors?.fullName && <p className='text-base text-red-500 pl-3'>{errors.fullName.message}</p>} */}
                </div>
              </div>
        ...
        <div className='flex flex-col gap-4'>
          <div className='bg-white p-4 shadow-md border rounded-xl'>
            <div>
              ...
              <div className='flex flex-col mb-3 space-y-2 w-[100%]'>
                ...
                <input  {...register('price', { value: product?.data.price })} type='number' placeholder='Base price' className='focus:outline-none rounded-xl  px-3 py-2 border w-full text-base focus:ring-2' />
                {/* {errors?.fullName && <p className='text-base text-red-500 pl-3'>{errors.fullName.message}</p>} */}
              </div>
              <div className='flex flex-col mb-3 space-y-2 w-[100%]'>
                <label className='text-base font-medium pl-3'>
                ...
                <input  {...register('salePrice', { value: product?.data.salePrice })} type='number' placeholder='Discount price' className='focus:outline-none rounded-xl  px-3 py-2 border w-full text-base focus:ring-2' />
                {/* {errors?.fullName && <p className='text-base text-red-500 pl-3'>{errors.fullName.message}</p>} */}
              </div>
            </div>
          </div>
          <div className='bg-white p-4 shadow-md border rounded-xl'>
            <div>
              ...
              <Space wrap>
                <Select
                  defaultValue={product?.data.category.categoryName}
                  // className='w-full'
                  style={{ width: 370 }}
                  onChange={(data) => setValue('categoryId', parseInt(data))}
